 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const lr=new kl("@firebase/firestore");function wr(){return lr.logLevel}function J(e,...t){if(lr.logLevel<=yt.DEBUG){const n=t.map(Fl);lr.debug(`Firestore (${Kr}): ${e}`,...n)}}function _n(e,...t){if(lr.logLevel<=yt.ERROR){const n=t.map(Fl);lr.error(`Firestore (${Kr}): ${e}`,...n)}}function Ln(e,...t){if(lr.logLevel<=yt.WARN){const n=t.map(Fl);lr.warn(`Firestore (${Kr}): ${e}`,...n)}}function Fl(e){if(typeof e=="string")return e;try{/**
* @license
* Copyright 2020 Google LLC
*
* Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const bI=new $t(rt.comparator);function qn(){return bI}const Hp=new $t(rt.comparator);function _s(...e){let t=Hp;for(const n of e)t=t.insert(n.key,n);return t}function RI(e){let t=Hp;return e.forEach((n,r)=>t=t.insert(n,r.overlayedDocument)),t}function er(){return xs()}function zp(){return xs()}function xs(){return new fr(e=>e.toString(),(e,t)=>e.isEqual(t))}const SI=new zt(rt.comparator);function Et(...e){let t=SI;for(const n of e)t=t.add(n);return t}const CI=new zt(ht);function PI(){return CI}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class TA{constructor(){this.As=!1,this.Rs=!1,this.Vs=100,this.fs=function(){return Ew()?8:eI(_w())>0?6:4}()}initialize(t,n){this.gs=t,this.indexManager=n,this.As=!0}getDocumentsMatchingQuery(t,n,r,s){const i={result:null};return this.ps(t,n).next(a=>{i.result=a}).next(()=>{if(!i.result)return this.ys(t,n,s,r).next(a=>{i.result=a})}).next(()=>{if(i.result)return;const a=new wA;return this.ws(t,n,a).next(l=>{if(i.result=l,this.Rs)return this.Ss(t,n,a,l.size)})}).next(()=>i.result)}Ss(t,n,r,s){return r.documentReadCount<this.Vs?(wr()<=yt.DEBUG&&J("QueryEngine","SDK will not create cache indexes for query:",Tr(n),"since it only creates cache indexes for collection contains","more than or equal to",this.Vs,"documents"),k.resolve()):(wr()<=yt.DEBUG&&J("QueryEngine","Query:",Tr(n),"scans",r.documentReadCount,"local documents and returns",s,"documents as results."),r.documentReadCount>this.fs*s?(wr()<=yt.DEBUG&&J("QueryEngine","The SDK decides to create cache indexes for query:",Tr(n),"as using cache indexes may help improve performance."),this.indexManager.createTargetIndexes(t,Ze(n))):k.resolve())}ps(t,n){if(kh(n))return k.resolve(null);let r=Ze(n);return this.indexManager.getIndexType(t,r).next(s=>s===0?null:(n.limit!==null&&s===1&&(n=Ja(n,null,"F"),r=Ze(n)),this.indexManager.getDocumentsMatchingTarget(t,r).next(i=>{const a=Et(...i);return this.gs.getDocuments(t,a).next(l=>this.indexManager.getMinOffset(t,r).next(c=>{const h=this.bs(n,l);return this.Ds(n,h,a,c.readTime)?this.ps(t,Ja(n,null,"F")):this.vs(t,h,n,c)}))})))}ys(t,n,r,s){return kh(n)||s.isEqual(at.min())?k.resolve(null):this.gs.getDocuments(t,r).next(i=>{const a=this.bs(n,i);return this.Ds(n,a,r,s)?k.resolve(null):(wr()<=yt.DEBUG&&J("QueryEngine","Re-using previous result from %s to execute query: %s",s.toString(),Tr(n)),this.vs(t,a,n,YT(s,qs)).next(l=>l))})}bs(t,n){let r=new zt(qp(t));return n.forEach((s,i)=>{bo(t,i)&&(r=r.add(i))}),r}Ds(t,n,r,s){if(t.limit===null)return!1;if(r.size!==n.size)return!0;const i=t.limitType==="F"?n.last():n.first();return!!i&&(i.hasPendingWrites||i.version.compareTo(s)>0)}ws(t,n,r){return wr()<=yt.DEBUG&&J("QueryEngine","Using full collection scan to execute query:",Tr(n)),this.gs.getDocumentsMatchingQuery(t,n,Un.min(),r)}vs(t,n,r,s){return this.gs.getDocumentsMatchingQuery(t,r,s).next(i=>(n.forEach(a=>{i=i.insert(a.key,a)}),i))}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const tb="firebase-installations-database",eb=1,ur="firebase-installations-store";let va=null;function hc(){return va||(va=cp(tb,eb,{upgrade:(e,t)=>{switch(t){case 0:e.createObjectStore(ur)}}})),va}async function to(e,t){const n=Oo(e),s=(await hc()).transaction(ur,"readwrite"),i=s.objectStore(ur),a=await i.get(n);return await i.put(t,n),await s.done,(!a||a.fid!==t.fid)&&Jg(e,t.fid),t}async function Zg(e){const t=Oo(e),r=(await hc()).transaction(ur,"readwrite");await r.objectStore(ur).delete(t),await r.done}async function No(e,t){const n=Oo(e),s=(await hc()).transaction(ur,"readwrite"),i=s.objectStore(ur),a=await i.get(n),l=t(a);return l===void 0?await i.delete(n):await i.put(l,n),await s.done,l&&(!a||a.fid!==l.fid)&&Jg(e,l.fid),l}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function fc(e){let t;const n=await No(e.appConfig,r=>{const s=nb(r),i=rb(e,s);return t=i.registrationPromise,i.installationEntry});return n.fid===cl?{installationEntry:await t}:{installationEntry:n,registrationPromise:t}}function nb(e){const t=e||{fid:Q0(),registrationStatus:0};return tm(t)}function rb(e,t){if(t.registrationStatus===0){if(!navigator.onLine){const s=Promise.reject(cr.create("app-offline"));return{installationEntry:t,registrationPromise:s}}const n={fid:t.fid,registrationStatus:1,registrationTime:Date.now()},r=sb(e,n);return{installationEntry:n,registrationPromise:r}}else return t.registrationStatus===1?{installationEntry:t,registrationPromise:ib(e)}:{installationEntry:t}}async function sb(e,t){try{const n=await G0(e,t);return to(e.appConfig,n)}catch(n){throw qg(n)&&n.customData.serverCode===409?await Zg(e.appConfig):await to(e.appConfig,{fid:t.fid,registrationStatus:0}),n}}async function ib(e){let t=await mf(e.appConfig);for(;t.registrationStatus===1;)await Qg(100),t=await mf(e.appConfig);if(t.registrationStatus===0){const{installationEntry:n,registrationPromise:r}=await fc(e);return r||n}return t}function mf(e){return No(e,t=>{if(!t)throw cr.create("installation-not-found");return tm(t)})}function tm(e){return ob(e)?{fid:e.fid,registrationStatus:0}:e}function ob(e){return e.registrationStatus===1&&e.registrationTime+$g<Date.now()}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
