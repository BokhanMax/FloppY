 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const lr=new Nl("@firebase/firestore");function Tr(){return lr.logLevel}function J(e,...t){if(lr.logLevel<=yt.DEBUG){const n=t.map(Ml);lr.debug(`Firestore (${Gr}): ${e}`,...n)}}function _n(e,...t){if(lr.logLevel<=yt.ERROR){const n=t.map(Ml);lr.error(`Firestore (${Gr}): ${e}`,...n)}}function Ln(e,...t){if(lr.logLevel<=yt.WARN){const n=t.map(Ml);lr.warn(`Firestore (${Gr}): ${e}`,...n)}}function Ml(e){if(typeof e=="string")return e;try{/**
* @license
* Copyright 2020 Google LLC
*
* Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const RI=new Bt(rt.comparator);function qn(){return RI}const Kp=new Bt(rt.comparator);function ms(...e){let t=Kp;for(const n of e)t=t.insert(n.key,n);return t}function SI(e){let t=Kp;return e.forEach((n,r)=>t=t.insert(n,r.overlayedDocument)),t}function nr(){return Vs()}function Wp(){return Vs()}function Vs(){return new fr(e=>e.toString(),(e,t)=>e.isEqual(t))}const CI=new zt(rt.comparator);function Et(...e){let t=CI;for(const n of e)t=t.add(n);return t}const PI=new zt(ht);function VI(){return PI}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class gA{constructor(){this.overlays=new Bt(rt.comparator),this.kr=new Map}getOverlay(t,n){return k.resolve(this.overlays.get(n))}getOverlays(t,n){const r=nr();return k.forEach(n,s=>this.getOverlay(t,s).next(i=>{i!==null&&r.set(s,i)})).next(()=>r)}saveOverlays(t,n,r){return r.forEach((s,i)=>{this.wt(t,n,i)}),k.resolve()}removeOverlaysForBatchId(t,n,r){const s=this.kr.get(r);return s!==void 0&&(s.forEach(i=>this.overlays=this.overlays.remove(i)),this.kr.delete(r)),k.resolve()}getOverlaysForCollection(t,n,r){const s=nr(),i=n.length+1,a=new rt(n.child("")),l=this.overlays.getIteratorFrom(a);for(;l.hasNext();){const c=l.getNext().value,h=c.getKey();if(!n.isPrefixOf(h.path))break;h.path.length===i&&c.largestBatchId>r&&s.set(c.getKey(),c)}return k.resolve(s)}getOverlaysForCollectionGroup(t,n,r,s){let i=new Bt((h,d)=>h-d);const a=this.overlays.getIterator();for(;a.hasNext();){const h=a.getNext().value;if(h.getKey().getCollectionGroup()===n&&h.largestBatchId>r){let d=i.get(h.largestBatchId);d===null&&(d=nr(),i=i.insert(h.largestBatchId,d)),d.set(h.getKey(),h)}}const l=nr(),c=i.getIterator();for(;c.hasNext()&&(c.getNext().value.forEach((h,d)=>l.set(h,d)),!(l.size()>=s)););return k.resolve(l)}wt(t,n,r){const s=this.overlays.get(r.key);if(s!==null){const a=this.kr.get(s.largestBatchId).delete(r.key);this.kr.set(s.largestBatchId,a)}this.overlays=this.overlays.insert(r.key,new UI(n,r));let i=this.kr.get(n);i===void 0&&(i=Et(),this.kr.set(n,i)),this.kr.set(n,i.add(r.key))}}/**
 * @license
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class IA{constructor(){this.As=!1,this.Rs=!1,this.Vs=100,this.fs=function(){return ww()?8:nI(yw())>0?6:4}()}initialize(t,n){this.gs=t,this.indexManager=n,this.As=!0}getDocumentsMatchingQuery(t,n,r,s){const i={result:null};return this.ps(t,n).next(a=>{i.result=a}).next(()=>{if(!i.result)return this.ys(t,n,s,r).next(a=>{i.result=a})}).next(()=>{if(i.result)return;const a=new TA;return this.ws(t,n,a).next(l=>{if(i.result=l,this.Rs)return this.Ss(t,n,a,l.size)})}).next(()=>i.result)}Ss(t,n,r,s){return r.documentReadCount<this.Vs?(Tr()<=yt.DEBUG&&J("QueryEngine","SDK will not create cache indexes for query:",Ir(n),"since it only creates cache indexes for collection contains","more than or equal to",this.Vs,"documents"),k.resolve()):(Tr()<=yt.DEBUG&&J("QueryEngine","Query:",Ir(n),"scans",r.documentReadCount,"local documents and returns",s,"documents as results."),r.documentReadCount>this.fs*s?(Tr()<=yt.DEBUG&&J("QueryEngine","The SDK decides to create cache indexes for query:",Ir(n),"as using cache indexes may help improve performance."),this.indexManager.createTargetIndexes(t,Ze(n))):k.resolve())}ps(t,n){if(Nh(n))return k.resolve(null);let r=Ze(n);return this.indexManager.getIndexType(t,r).next(s=>s===0?null:(n.limit!==null&&s===1&&(n=Ja(n,null,"F"),r=Ze(n)),this.indexManager.getDocumentsMatchingTarget(t,r).next(i=>{const a=Et(...i);return this.gs.getDocuments(t,a).next(l=>this.indexManager.getMinOffset(t,r).next(c=>{const h=this.bs(n,l);return this.Ds(n,h,a,c.readTime)?this.ps(t,Ja(n,null,"F")):this.vs(t,h,n,c)}))})))}ys(t,n,r,s){return Nh(n)||s.isEqual(ot.min())?k.resolve(null):this.gs.getDocuments(t,r).next(i=>{const a=this.bs(n,i);return this.Ds(n,a,r,s)?k.resolve(null):(Tr()<=yt.DEBUG&&J("QueryEngine","Re-using previous result from %s to execute query: %s",s.toString(),Ir(n)),this.vs(t,a,n,JT(s,js)).next(l=>l))})}bs(t,n){let r=new zt(Gp(t));return n.forEach((s,i)=>{bo(t,i)&&(r=r.add(i))}),r}Ds(t,n,r,s){if(t.limit===null)return!1;if(r.size!==n.size)return!0;const i=t.limitType==="F"?n.last():n.first();return!!i&&(i.hasPendingWrites||i.version.compareTo(s)>0)}ws(t,n,r){return Tr()<=yt.DEBUG&&J("QueryEngine","Using full collection scan to execute query:",Ir(n)),this.gs.getDocumentsMatchingQuery(t,n,Un.min(),r)}vs(t,n,r,s){return this.gs.getDocumentsMatchingQuery(t,r,s).next(i=>(n.forEach(a=>{i=i.insert(a.key,a)}),i))}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Qg=new Map;function Yg(e,t){const n=Oo(e);Jg(n,t),X0(n,t)}function Jg(e,t){const n=Qg.get(e);if(n)for(const r of n)r(t)}function X0(e,t){const n=Z0();n&&n.postMessage({key:e,fid:t}),tb()}let rr=null;function Z0(){return!rr&&"BroadcastChannel"in self&&(rr=new BroadcastChannel("[Firebase] FID Change"),rr.onmessage=e=>{Jg(e.data.key,e.data.fid)}),rr}function tb(){Qg.size===0&&rr&&(rr.close(),rr=null)}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const eb="firebase-installations-database",nb=1,ur="firebase-installations-store";let va=null;function uc(){return va||(va=fp(eb,nb,{upgrade:(e,t)=>{switch(t){case 0:e.createObjectStore(ur)}}})),va}async function Zi(e,t){const n=Oo(e),s=(await uc()).transaction(ur,"readwrite"),i=s.objectStore(ur),a=await i.get(n);return await i.put(t,n),await s.done,(!a||a.fid!==t.fid)&&Yg(e,t.fid),t}async function Xg(e){const t=Oo(e),r=(await uc()).transaction(ur,"readwrite");await r.objectStore(ur).delete(t),await r.done}async function No(e,t){const n=Oo(e),s=(await uc()).transaction(ur,"readwrite"),i=s.objectStore(ur),a=await i.get(n),l=t(a);return l===void 0?await i.delete(n):await i.put(l,n),await s.done,l&&(!a||a.fid!==l.fid)&&Yg(e,l.fid),l}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
