...
<template>
  <div
    class="bg-green-50 rounded-lg shadow-md 2xl:p-5 lg:p-3 xs:p-3 hover:shadow-lg transition-shadow"
    :id="props.id"
  >
    <div class="flex items-center place-content-between mb-4">
      <div class="flex">
        ...
        >
          ...
          <picture>
            ...
            />
            <v-lazy-image
              :src="'icons/fallback/' + props.name.replace(/\s+/g, '') + 'Icon.png'"
              :alt="name"
              :title="'Скачати ' + props.name + ' безкоштовно по прямому посиланню'"
            />
          </picture>
      ...
      <div class="flex self-start flex-col items-end">
        ...
        <a
          v-if="website"
          :href="props.website"
          class="text-gray-600 md:text-xs lg:text-[8px] xl:text-xs xs:text-[10px]"
          target="_blank"
        >
          🌐 Сайт
        </a>
